...
JSONLD_DEFAULT_CHUNK_SIZE :: 64 * 1024
JSONLD_MAX_EMPTY_READS    :: 100

// Reader_Options controls bounded buffering for a JSON-LD document. JSON-LD
// semantics require the complete document, so max_document_bytes is an actual
// retained-document bound rather than a per-record framing limit.
...
// Reader_Result returns the JSON-LD outcome, the underlying reader error, and
// the number of dataset statements passed to the caller's sink.
...
// parse_reader reads one bounded JSON-LD document, then transforms it to RDF.
// Reader ownership remains with the caller. It intentionally does not claim
// streaming memory use: the document is retained for JSON-LD processing.
parse_reader :: proc(reader: io.Reader, sink: Sink, options: Reader_Options = {}, user_data: rawptr = nil) -> Reader_Result {
	...
	if chunk_size == 0 do chunk_size = JSONLD_DEFAULT_CHUNK_SIZE
	maximum := options.max_document_bytes
	if maximum == 0 do maximum = options.parse.max_document_bytes
	...
	for {
		...
		if count == 0 && read_error == .None {
			empty_reads += 1
			if empty_reads >= JSONLD_MAX_EMPTY_READS {
	...
	parse_options.max_document_bytes = maximum
	state := Reader_State{sink = sink, user_data = user_data}
	// core:encoding/json's tokenizer relies on a NUL-terminated backing buffer
	// on some reader-backed paths; clone gives it owned termination explicitly.
	...
	result.error = parse(document, forward_quad, parse_options, &state)
