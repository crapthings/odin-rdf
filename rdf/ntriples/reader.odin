...
@(private) parse_reader_line :: proc(line: []byte, line_number: int, state: ^Reader_State) -> Parse_Error {
	err := parse_with_scope(string(line), reader_sink, state.scope, state)
...
parse_reader :: proc(
	...
	defer delete(line)
	state := Reader_State{sink = sink, user_data = user_data, max_triples = options.max_triples, scope = new_blank_node_scope()}
	line_number := 1
	skip_lf := false
	no_progress := 0
