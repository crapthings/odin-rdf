...
// DateTime_Value is the parsed XML Schema dateTime value used solely for
// exact value comparison. The year remains an arbitrary-width decimal string.
@(private) DateTime_Value :: struct {
	year_negative: bool,
	year:          string,
	month, day:    int,
	hour, minute:  int,
...
// xsd:dateTimeStamp values. Explicit timezones are normalized to UTC; values
// with and without an explicit timezone are distinct value categories.
owl_rl_datetime_literals_have_same_value :: proc(left, right: Term) -> (compared, same: bool) {
	...
	left_value, left_ok := parse_owl_rl_datetime_value(left.value)
	right_value, right_ok := parse_owl_rl_datetime_value(right.value)
	if !left_ok || !right_ok do return false, false
	if left_value.has_timezone != right_value.has_timezone do return true, false

	left_shift := 0
	right_shift := 0
	left_minutes := left_value.hour * 60 + left_value.minute
	right_minutes := right_value.hour * 60 + right_value.minute
	if left_value.hour == 24 { left_minutes = 0; left_shift = 1 }
	if right_value.hour == 24 { right_minutes = 0; right_shift = 1 }
	if left_value.has_timezone {
		left_minutes -= left_value.utc_offset
		right_minutes -= right_value.utc_offset
		for left_minutes < 0 { left_minutes += 24 * 60; left_shift -= 1 }
		for left_minutes >= 24 * 60 { left_minutes -= 24 * 60; left_shift += 1 }
		for right_minutes < 0 { right_minutes += 24 * 60; right_shift -= 1 }
		for right_minutes >= 24 * 60 { right_minutes -= 24 * 60; right_shift += 1 }
	}
	if !datetime_date_shift_equals(left_value, left_shift, right_value, right_shift) do return true, false
	if left_minutes != right_minutes || left_value.second != right_value.second do return true, false
	return true, datetime_fraction_equal(left_value.fraction, right_value.fraction)
}

@(private) parse_owl_rl_datetime_value :: proc(value: string) -> (DateTime_Value, bool) {
	...
	result := DateTime_Value{year_negative = negative, year = year, month = month, day = day, hour = hour, minute = minute, second = second, fraction = fraction}
...
@(private) datetime_date_shift_equals :: proc(left: DateTime_Value, left_shift: int, right: DateTime_Value, right_shift: int) -> bool {
...
// datetime_shifted_date_equals determines whether source plus at most two
// calendar days equals target. Timezone normalization can shift each side by
// at most one day, making this bounded comparison sufficient.
@(private) datetime_shifted_date_equals :: proc(source: DateTime_Value, days: int, target: DateTime_Value) -> bool {
...
@(private) datetime_base_date_equal :: proc(left, right: DateTime_Value) -> bool {
