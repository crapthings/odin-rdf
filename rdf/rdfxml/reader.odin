...
RDFXML_DEFAULT_CHUNK_SIZE :: 64 * 1024
RDFXML_MAX_EMPTY_READS    :: 100

// Reader_Options controls bounded buffering for an RDF/XML document. RDF/XML
// namespace and node processing require one retained XML document.
...
// Reader_Result returns the RDF/XML outcome, underlying reader error, and
// number of default-graph statements passed to the caller's sink.
...
// parse_reader reads one bounded RDF/XML document, then transforms it to RDF.
...
parse_reader :: proc(reader: io.Reader, sink: Sink, options: Reader_Options = {}, user_data: rawptr = nil) -> Reader_Result {
	...
	if chunk_size == 0 do chunk_size = RDFXML_DEFAULT_CHUNK_SIZE
	...
	for {
		...
		if count == 0 && read_error == .None {
			empty_reads += 1
			if empty_reads >= RDFXML_MAX_EMPTY_READS {
	...
	// Give the XML parser an owned, NUL-terminated string independent from the
	// reusable reader buffer.
