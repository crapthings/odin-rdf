...
Error_Code :: enum {
	...
	Invalid_Option,
	Invalid_Chunk_Size,
	Invalid_Statement_Limit,
	Prefix_Limit,
	Prefix_Bytes_Limit,
	Token_Limit,
	Nesting_Limit,
	Statement_Too_Long,
	Pending_Triple_Limit,
	Triple_Limit,
	Out_Of_Memory,
	Reader_Error,
	No_Progress,
...
parse_error_message :: proc(code: Error_Code) -> string {
	...
	case .Expected_Subject:            return "expected Turtle subject"
	case .Expected_Predicate:          return "expected Turtle predicate"
	case .Expected_Object:             return "expected Turtle object"
	...
	case .Invalid_Statement_Limit:     return "statement limit must not be negative"
	...
	case .Statement_Too_Long:          return "statement exceeds configured limit"
	case .Pending_Triple_Limit:        return "pending triple limit reached"
...
// Sink receives triples after their complete top-level Turtle statement has
...
// select documented defaults, except max_triples where zero disables the limit.
Parse_Options :: struct {
	...
	// Maximum expanded triples buffered before one statement commits.
	max_pending_triples: int,
	// Maximum triples emitted by the document. Zero disables the limit.
	max_triples:         int,
}
...
DEFAULT_MAX_PENDING_TRIPLES :: 100_000
...
@(private) read_prefixed_name :: proc(p: ^Parser) -> (rdf.Term, Parse_Error) {
	...
	for s.pos < len(s.input) {
		...
		if c == '.' {
			...
			if next == ' ' || next == '\t' || next == '\r' || next == '\n' || next == ';' || next == ',' || next == ']' || next == ')' || next == '#' do break
			strings.write_byte(&builder, c)
			termlex.advance_ascii(s)
			continue
		}
...
@(private) looks_like_prefixed_name :: proc(p: ^Parser) -> bool {
	...
	for i := s.pos; i < len(s.input); i += 1 {
		c := s.input[i]
		if c == ':' do return true
		if c <= ' ' || c == ';' || c == ',' || c == '[' || c == '(' || c == '<' || c == '>' || c == '"' || c == '\'' do return false
	}
	return false
}
...
@(private) fresh_blank_node :: proc(p: ^Parser) -> (rdf.Term, Parse_Error) {
	...
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	strings.write_string(&builder, ".turtle-genid-")
...
@(private) read_collection :: proc(p: ^Parser) -> (rdf.Term, Parse_Error) {
	if err := enter_nesting(p); err.code != .None do return {}, err
	defer p.depth -= 1
	termlex.advance_ascii(&p.scanner)
	if err := skip_space_and_comments(p); err.code != .None do return {}, err
...
@(private) append_pending :: proc(p: ^Parser, triple: rdf.Triple) -> Parse_Error {
	if len(p.pending) >= p.max_pending_triples do return error_at(p, .Pending_Triple_Limit)
	append(&p.pending, triple)
	return {}
}
...
@(private) parse_document :: proc(p: ^Parser) -> Parse_Error {
	for {
		...
		if err := skip_space_and_comments(p); err.code != .None do return err
		if !(p.property_subject && p.scanner.pos < len(p.scanner.input) && p.scanner.input[p.scanner.pos] == '.') {
			if err := parse_predicate_object_list(p, subject); err.code != .None do return err
		}
		if err := skip_space_and_comments(p); err.code != .None do return err
		if p.scanner.pos >= len(p.scanner.input) || p.scanner.input[p.scanner.pos] != '.' do return error_at(p, .Expected_Dot)
		termlex.advance_ascii(&p.scanner)
		if p.max_triples > 0 && p.emitted + len(p.pending) > p.max_triples do return error_at(p, .Triple_Limit)
		for triple in p.pending {
			if !p.sink(triple, p.user_data) do return error_at(p, .Stopped)
			p.emitted += 1
		}
		clear_owned(p)
	}
...
@(private) init_parser :: proc(input: string, sink: Sink, options: Parse_Options, user_data: rawptr) -> (Parser, Parse_Error) {
	...
	if options.max_token_bytes < 0 || options.max_prefixes < 0 || options.max_prefix_bytes < 0 || options.max_nesting_depth < 0 || options.max_pending_triples < 0 || options.max_triples < 0 {
	...
	p := Parser{
		...
		pending = make([dynamic]rdf.Triple),
		...
		max_pending_triples = options.max_pending_triples,
	...
	if p.max_pending_triples == 0 do p.max_pending_triples = DEFAULT_MAX_PENDING_TRIPLES
...
// parse parses a complete RDF 1.1 Turtle document already held in memory.
